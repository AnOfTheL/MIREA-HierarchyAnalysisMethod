    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 2; i < 10; i++)
            {
                ...
                //arrayItem[i + 7] = string.Format("{0:N2}", 1.0 / double.Parse(i.ToString()));
            }
        ...
        private void scaleComboBox_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            if (elementComboBox.SelectedIndex == 0) { allCriterionsComparing.flag = false; }
            else { criterionsList[criterionsComboBox.SelectedIndex].flag = false; }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (columnIndex == 0 || (columnIndex - 1) == rowIndex) { return; }
        ...
        {
            Criterion element;
            if (elementComboBox.SelectedIndex == 0) { element = allCriterionsComparing; }
            else { element = criterionsList[criterionsComboBox.SelectedIndex]; }
            ...
            else
            {
                ...
                if (element.OS < 0.1) { resultTextBox.Text = "Матрица согласована, т.к. ОС < 0.1"; }
                else { resultTextBox.Text = "Матрица не согласована, т.к. ОС > 0.1"; }
            }

            if (elementComboBox.SelectedIndex == 0) { allCriterionsComparing.flag = true; }
            else { criterionsList[criterionsComboBox.SelectedIndex].flag = true; }
        }

        private void resultButton_Click(object sender, EventArgs e)
        {
            if(!allCriterionsComparing.CheckExploring()) { return; }
            foreach(Criterion element in criterionsList)
            {
                if(!element.CheckExploring()) { return; }
            }
